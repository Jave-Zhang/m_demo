...
def soft_sign(x):
    """
    Implement a smooth sign function using sigmoid with a specific steepness.
    
    Args:
        x: Input tensor
        
    Returns:
        torch.Tensor: Smoothed sign values in range (0,1)
    """
    k = 1
...
def postprocess_new(u, x, lr_min, lr_max, num_itr, rho=0.0, with_l1=False, s=math.log(9.0)):
    """
    Postprocess the utility matrix to enforce RNA structure constraints.
    
    Args:
        u: Utility matrix (logits), assumed to be symmetric, in batch
        x: RNA sequence in one-hot encoding, in batch
        lr_min: Learning rate for minimization step
        lr_max: Learning rate for maximization step (for lagrangian multiplier)
        num_itr: Number of iterations
        rho: Sparsity coefficient
        with_l1: Whether to apply L1 regularization
        s: Sigmoid threshold, default log(9.0) corresponds to prob > 0.9
        
    Returns:
        torch.Tensor: Optimized contact map
    """
    # Get constraint matrix based on nucleotide pairing possibilities
    m = constraint_matrix_batch(x).float().to("cuda:1")
    
    # Apply soft thresholding to utility matrix
    u = soft_sign(u - s) * u

    # Initialization
    a_hat = (torch.sigmoid(u)) * soft_sign(u - s).detach().to("cuda:1")
    ...
    for t in range(num_itr):
        ...
        # Apply L1 regularization if requested
        if with_l1:
...
def postprocess_new_nc(u, x, lr_min, lr_max, num_itr, rho=0.0, with_l1=False,s=math.log(9.0)):
    ...
    :return:
    """
    m = constraint_matrix_batch_addnc(x).float().to("cuda:1")
    #m = 1.0
    # u with threshold
